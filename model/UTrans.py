...
class conv_block(nn.Module):
    ...
    def __init__(self, in_ch, out_ch,use_eql=True):
        ...
        if use_eql:
            ...
            self.conv_1 = nn.Conv2d(in_ch, out_ch, (3, 3),
                                 padding=1, bias=True)
...
class Block_ViT(nn.Module):
    def __init__(self, vis, channel_num, expand_ratio=4,KV_size=480):
        ...
        self.attn_norm2 = nn.LayerNorm(channel_num[1],eps=1e-6)
        self.attn_norm3 = nn.LayerNorm(channel_num[2],eps=1e-6)
        self.attn_norm4 = nn.LayerNorm(channel_num[3],eps=1e-6)
        self.attn_norm =  nn.LayerNorm(KV_size,eps=1e-6)
        self.channel_attn = Attention_org(vis, channel_num)

        self.ffn_norm1 = nn.LayerNorm(channel_num[0],eps=1e-6)
        self.ffn_norm2 = nn.LayerNorm(channel_num[1],eps=1e-6)
        self.ffn_norm3 = nn.LayerNorm(channel_num[2],eps=1e-6)
        self.ffn_norm4 = nn.LayerNorm(channel_num[3],eps=1e-6)
...
class Encoder(nn.Module):
    def __init__(self, vis, channel_num, num_layers=4):
        ...
        self.encoder_norm1 = nn.LayerNorm(channel_num[0],eps=1e-6)
        self.encoder_norm2 = nn.LayerNorm(channel_num[1],eps=1e-6)
        self.encoder_norm3 = nn.LayerNorm(channel_num[2],eps=1e-6)
        self.encoder_norm4 = nn.LayerNorm(channel_num[3],eps=1e-6)
...
class UTrans(nn.Module):
	def __init__(
		...
		#modulelist
		self.rgb_to_feature = nn.ModuleList([from_rgb(32),from_rgb(64),from_rgb(128)])
		self.feature_to_rgb = nn.ModuleList([to_rgb(32),to_rgb(64),to_rgb(128),to_rgb(256)])
